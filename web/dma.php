<?php
    ...
    class Dma
    {
        ...
        {
            session('dma')->setAuth(null);
            Router::redirect(container()->getUrlsite() . 'dma.php');
        }

        private function home()
        {
            return '<a href="' . container()->getUrlsite() . 'dma.php?action=addTable">
        ...
        {
            $html = '<h3 class="titleTable">' . $this->tableByName('dma_table')->find($id)->getName() . '</h3>';
            $first = $this->action != 'displayData'
            ? '<a href="' . container()->getUrlsite() . 'dma.php?action=displayData&amp;id=' . $id . '"><i class="fa fa-folder-open"></i> Display data</a>'
            : '<a href="' . container()->getUrlsite() . 'dma.php?action=table&amp;id=' . $id . '"><i class="fa fa-tasks"></i> Display structures</a>';
            $html .= '<p>
                ' . $first . ' |
                <a href="' . container()->getUrlsite() . 'dma.php?action=addRecord&amp;id=' . $id . '"><i class="fa fa-plus-square"></i> Add record</a> |
                <a href="' . container()->getUrlsite() . 'dma.php?action=import&amp;table=' . $id . '"><i class="fa fa-mail-forward"></i> Import data</a> |
                <a href="' . container()->getUrlsite() . 'dma.php?action=emptyTable&amp;id=' . $id . '"><i class="fa fa-ban"></i> Empty the table</a> |
                <a href="' . container()->getUrlsite() . 'dma.php?action=removeTable&amp;id=' . $id . '"><i class="fa fa-trash-o"></i> Delete the table</a>
            </p>';
            return $html;
        }
        ...
        private function deleteRecordConfirm()
        {
            ...
            if (strlen($id) && strlen($table) && strlen($token)) {
                if ($token == sha1('deleteRecord' . $id . $table . date('dmY'))) {
                    $db     = $this->tableByName($this->tableByName('dma_table')->find($table)->getName());
                    $obj    = $db->find($id);
                    if (!empty($obj)) {
                        $obj->delete();
                        header('Location: ' . container()->getUrlsite() . 'dma.php?action=displayData&id=' . $table .'');
                    }
                } else {
        ...
        private function deleteStructureConfirm()
        {
            ...
            if (strlen($id) && strlen($token)) {
                if ($token == sha1('deleteStructure' . $id . date('dmY'))) {
                    $dbs = $this->tableByName('dma_structure');
                    $obj = $dbs->find($id);
                    if (!empty($obj)) {
                        $db = $this->tableByName(
                            $this->tableByName('dma_table')->find(
                                $obj->getTable()
                            )->getName()
                        );
                        $data = $db->fetch()->exec();
                        ...
                        $obj->delete();
                        header('Location: ' . container()->getUrlsite() . 'dma.php?action=table&id=' . $obj->getTable() .'');
                    }
                } else {
                    return $this->error('Wrong token.');
                }
        ...
        private function deleteStructure()
        {
            ...
            if (strlen($id)) {
                ...
                if (!empty($obj)) {
                    ...
                    Click <a href="' . container()->getUrlsite() . 'dma.php?action=deleteStructureConfirm&amp;id=' . $id .'&amp;token=' . $key .'">here</a> to confirm this deletion or <a href="' . container()->getUrlsite() . 'dma.php?action=table&amp;id=' . $obj->getTable() .'">here</a> to go back.
                    </p>';
                } else {
                    return $this->error('Wrong request.');
                }
        ...
        private function deleteRecord()
        {
            ...
            if (strlen($id) && strlen($table)) {
                ...
                return '<p class="alert">
                Click <a href="' . container()->getUrlsite() . 'dma.php?action=deleteRecordConfirm&amp;table=' . $table .'&amp;id=' . $id .'&amp;token=' . $key .'">here</a> to confirm this deletion or <a href="' . container()->getUrlsite() . 'dma.php?action=displayData&amp;id=' . $table .'">here</a> to go back.
                </p>';
            } else {
                return $this->error('Wrong request.');
            }
        ...
        private function emptyTable()
        {
            $id     = isAke($_REQUEST, 'id', null);
            if (strlen($id)) {
                $table  = $this->tableByName('dma_table')->find($id);
                if (!empty($table)) {
                    $key = sha1('emptyTable' . $id . date('dmY'));
                    return '<p class="alert">
                    Click <a href="' . container()->getUrlsite() . 'dma.php?action=emptyTableConfirm&amp;id=' . $id .'&amp;token=' . $key .'">here</a> to empty the table &laquo; ' . $this->tableByName('dma_table')->find(request()->getId())->getName() . ' &raquo, or <a href="' . container()->getUrlsite() . 'dma.php?action=displayData&amp;id=' . $id .'">here</a> to go back.
                    </p>';
                } else {
                    return $this->error('Wrong request.');
                }
        ...
        private function removeTable()
        {
            ...
            if (strlen($id)) {
                ...
                if (!empty($table)) {
                    ...
                    return '<p class="alert">
                    Click <a href="' . container()->getUrlsite() . 'dma.php?action=removeTableConfirm&amp;id=' . $id .'&amp;token=' . $key .'">here</a> to drop the table &laquo; ' . $this->tableByName('dma_table')->find(request()->getId())->getName() . ' &raquo, or <a href="' . container()->getUrlsite() . 'dma.php?action=displayData&amp;id=' . $id .'">here</a> to go back.
                    </p>';
                } else {
                    return $this->error('Wrong request.');
                }
        ...
        private function addTable()
        {
            ...
            $html .= '<p>
            <form action="' . container()->getUrlsite() . 'dma.php?action=addTable" method="post" id="addTable">
            ...
            if (strlen($id) && strlen($token)) {
                ...
                if (!empty($table)) {
                    if ($token == sha1('removeTable' . $id . date('dmY'))) {
                        ...
                        $table->delete();
                        header(
                            'Location: ' . container()->getUrlsite() . 'dma.php'
                        );
                    } else {
                        return $this->error('Wrong request.');
                    }
            ...
            if (strlen($id) && strlen($token)) {
                ...
                if (!empty($table)) {
                    if ($token == sha1('emptyTable' . $id . date('dmY'))) {
                        ...
                        $db->emptyTable();
                        header(
                            'Location: ' . container()->getUrlsite() . 'dma.php?action=displayData&id=' . $id
                        );
                    } else {
                        return $this->error('Wrong request.');
                    }
        ...
        {
            $t      = $this->tableByName('dma_table')->find(request()->getId());
            if (empty($t)) {
            ...
            $html .= '<form id="listForm" action="' . container()->getUrlsite() . 'dma.php?action=displayData&id=' . request()->getId() . '" method="post">';
            ...
            foreach ($res as $row) {
                ...
                foreach ($fields as $k => $field) {
                    ...
                    if (strstr($type, 'fk_')) {
                        ...
                        $fk = container()->dbm($fkTable)->find($value);
                        if (!empty($fk)) {
                            $value = '<a class="fk" href="' . container()->getUrlsite() . 'dma.php?action=editRecord&amp;id=' . $fk->getId() . '&amp;table=' . $tableFk->getId() . '">' . $this->truncate($fk->string()) . '</a>';
                        }
                    }
                ...
                $html .= '<td>
                    <a href="' . container()->getUrlsite() . 'dma.php?action=editRecord&amp;id=' . $row->getId() . '&amp;table=' . request()->getId() . '"><i rel="tooltip" title="Edit record" class="fa fa-edit"></i></a> |
                    <a href="' . container()->getUrlsite() . 'dma.php?action=duplicateRecord&amp;id=' . $row->getId() . '&amp;table=' . request()->getId() . '"><i rel="tooltip" title="Duplicate record" class="fa fa-copy"></i></a> |
                    <a href="' . container()->getUrlsite() . 'dma.php?action=deleteRecord&amp;id=' . $row->getId() . '&amp;table=' . request()->getId() . '"><i rel="tooltip" title="Delete record" class="fa fa-trash-o"></i></a>
                </td></tr>';
            }
        ...
        private function table()
        {
            ...
            foreach ($structures as $structure) {
                ...
                $html .= '<td>
                    <a href="' . container()->getUrlsite() . 'dma.php?action=editStructure&amp;id=' . $s->getId() . '"><i rel="tooltip" title="Edit structure" class="fa fa-edit"></i></a> |
                    <a href="' . container()->getUrlsite() . 'dma.php?action=deleteStructure&amp;id=' . $s->getId() . '"><i rel="tooltip" title="Delete structure" class="fa fa-trash-o"></i></a>
                </td>';
                $html .= '</tr>';
            }
            $html .= '</table>';
            $html .= '<p><a href="' . container()->getUrlsite() . 'dma.php?action=addStructure&amp;table=' . request()->getId() . '"><i class="fa fa-plus"></i> Add a field</a></p>';
            return $html;
        }
        ...
        private function inputForm($type, $field, $length, $required, $value = null, $structure)
        {
            ...
            if (strstr($type, 'fk_')) {
                ...
                $select .= '</select>
                <a target="_plus" href="' . container()->getUrlsite() . 'dma.php?action=addRecord&amp;id=' . $tableFk->getId() . '">
        ...
        {
            $dbTable    = container()->dbm('dma_table');
            ...
            $html .= '<p class="first">
                <form action="' . container()->getUrlsite() . 'dma.php?action=import&amp;table=' . $table->getId() . '" method="post" id="import" enctype="multipart/form-data">
        ...
        private function importMap()
        {
            if (true === context()->isPost()) {
                ...
                $db         = container()->dbm($table->getName());
                ...
                Router::redirect(
                    container()->getUrlsite() . 'dma.php?action=displayData&id=' . $table->getId()
                );
            } else {
                return $this->error("An error occured. Please try again.");
            }
        ...
        private function map($structures, $table, $separator = '%%')
        {
            ...
            <form action="' . container()->getUrlsite() . 'dma.php?action=importMap&amp;table=' . $table->getId() . '" method="post" name="map" id="map"><input type="hidden" name="separator" id="separator" value="' . $separator . '" />
            ';
        ...
        private function editRecord()
        {
            ...
            $html .= '<form action="' . container()->getUrlsite() . 'dma.php?action=editRecord&amp;table=' . $table->getId() . '&amp;id=' . $record->getId() . '" method="post" id="editRecord" enctype="multipart/form-data">
            <input type="hidden" name="MAX_FILE_SIZE" value="100000000" />
            <table class="table">';
        ...
        private function duplicateRecord()
        {
            ...
            $html .= '<form action="' . container()->getUrlsite() . 'dma.php?action=duplicateRecord&amp;table=' . $table->getId() . '&amp;id=' . $record->getId() . '" method="post" id="duplicateRecord" enctype="multipart/form-data">
            <input type="hidden" name="MAX_FILE_SIZE" value="100000000" />
            <table class="table">';
            foreach ($structures as $structure) {
                $field      = $structure->field()->getName();
        ...
        private function addRecord()
        {
            ...
            $structures = $dbs->where('table = ' . $table->getId())->order('field')->exec(true);
            $html .= '<form action="' . container()->getUrlsite() . 'dma.php?action=addRecord&amp;id=' . $table->getId() . '" method="post" id="addRecord" enctype="multipart/form-data">
            <input type="hidden" name="MAX_FILE_SIZE" value="100000000" />
            <table class="table">';
            foreach ($structures as $structure) {
                $field      = $structure->field()->getName();
        ...
        private function addStructure()
        {
            ...
            if (true === context()->isPost()) {
                ...
                Router::redirect(
                    container()->getUrlsite() . 'dma.php?action=table&id=' . request()->getTable()
                );
            }
            ...
            $html           .= '<p class="first">
            <form action="' . container()->getUrlsite() . 'dma.php?action=addStructure&amp;table=' . request()->getTable() . '" method="post" id="editStructure">
            <table class="table">
            <tr>
                <th>Name</th>
                <th>Type</th>
        ...
        private function editStructure()
        {
            ...
            if (true === context()->isPost()) {
                ...
                $structure->hydrate()->save();
                Router::redirect(
                    container()->getUrlsite() . 'dma.php?action=table&id=' . $structure->getTable()
                );
            }
            ...
            $html .= '<p class="first">
            <form action="' . container()->getUrlsite() . 'dma.php?action=editStructure&amp;id=' . request()->getId() . '" method="post" id="editStructure">
            <table class="table">
            <tr>
                <th>Type</th>
                <th>Length</th>
        ...
        {
            $dbt = $this->tableByName('dma_table');
            $t = $dbt->find($id);
            if (!empty($t)) {
    ...
    if (true === $isAuth) {
        ...
        $dma        = new Dma();
        $content    = $dma->content($action, $tables);
    }
    ...
    <body>
        <div class="container">
            ...
            <h1 class="text-center">
                <span onclick="document.location.href = '<?php echo container()->getUrlsite(); ?>dma.php';" class="title">
                    <i class="fa fa-cogs fa-3x"></i>&nbsp;&nbsp;dataMyAdmin
                </span>
            </h1>
            ...
            <div class="row">
                ...
                <div class="pull-right">
                    <a href="<?php echo container()->getUrlsite(); ?>dma.php?action=home">
                        <i class="fa fa-home"></i>
                    </a> |
                    <a href="<?php echo container()->getUrlsite(); ?>dma.php?action=logout">
                ...
                <div class="col-md-2">
                    ...
                    <div id="tables">
                        <ul class="listTable">
                            ...
                            <li>
                                <i rel="tooltip-t" title="Show data" onclick="document.location.href = '<?php echo container()->getUrlsite(); ?>dma.php?action=displayData&amp;id=<?php echo $table['id']; ?>';" class="linkTable link fa fa-list"></i>

                                <a class="linkTable" href="<?php echo container()->getUrlsite(); ?>dma.php?action=table&amp;id=<?php echo $table['id']; ?>">
                                    <?php echo $dma->truncate($table['name']); ?>
                                </a>
                            </li>
                            <?php endforeach; ?>
                        </ul>
